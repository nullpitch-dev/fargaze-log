...
export function StackedBars({
  ...
  formatY?: (v: number) => string;
	...
	const [active, setActive] = useState<{ b: number; s: string } | null>(null);
  ...
  return (
    <div className="flex flex-col gap-1 w-full select-none">
            ...
            return (
              <div key={b.label} className="relative flex-1" style={{ margin: '0 3px' }}>
                {series.map(ser => {
                  ...
                  const bottom = acc;
                  acc += hPct;
									const isA = active?.b === bi && active?.s === ser.key;
                  const op = legendKey
                    ? (legendKey === ser.key ? 1 : 0.2)
                    : (isA ? 1 : 0.85);
                  return (
                    <div key={ser.key}
                      onMouseEnter={() => setActive({ b: bi, s: ser.key })}
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1 text-[12px]" style={{ color: lc }}>
          ...
          const dim = legendKey !== null && legendKey !== ser.key;
          return (
            <span key={ser.key}
              onMouseEnter={() => setLegendKey(ser.key)}
              onMouseLeave={() => setLegendKey(null)}
              className="inline-flex items-center gap-1 cursor-pointer"
              style={{ opacity: dim ? 0.4 : 1, transition: 'opacity 150ms ease' }}>
