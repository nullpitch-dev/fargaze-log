...
C(table(["Component","File","Description"],[
  ...
  ["dotSize(n)","src/app/insights/_components/charts/css-chart-components.tsx","(v4.6) Shared per-bucket dot diameter ladder \u2014 7px to 16 buckets, then 6, 5, 4, and 3px above 100, with a +2px bump on the hovered dot at every size. At 120 buckets a 7px dot is wider than its slot and the line reads as a string of beads. Drives every dot in CssTrendChart and CssDualLineChart, right-axis dots included; nothing changes below 16 buckets"],
...
C(
  ...
  bullet("(v4.6) Converted to the Weight-style grain \u00d7 count window, the first widget to use the shared trend-window.ts. New metric=interactions.trend fetches ONCE for the whole window with a one-day pad each side against timezone edge shift and buckets in memory, delegating each bucket to the existing computeInteractionsTrendBucket so the old and new paths cannot disagree while both exist."),
  bullet("Relation and Method moved from stacked bars to stacked areas; the People tab keeps its own 3/6/12 count. Same controls, same server-echoed grain rule and same Period-mode behaviour as Drinking below.", 1),
);
C(bold("Filters"));
C(
...
C(bold("Rest Day Score & Histogram Buckets (REDESIGNED v4.6)"));
C(p("A rest score belongs to a DRINKING DAY only, and equals the number of dry days directly before it. Non-drinking days carry no score at all. Drinking on a Sunday, staying dry all week and drinking the next Sunday scores 6 \u2014 one case, not seven."));
C(
  bullet("A drinking day whose previous drinking day falls before the window is scored from that earlier day. Finding it needs the single most recent drinking day before the window start \u2014 one bounded indexed lookup, which is also what replaces the all-time scan (see the Trend API below)."),
  bullet("The very first drinking day in the whole log carries NO score and is excluded from both the histogram and the average: the length of the dry run before logging began is unknown, so any number there would be invented."),
  bullet("A bucket with no drinking day has an UNDEFINED average, not a zero. avgRestDays is null and the chart breaks the line there."),
  bullet("Such a bucket instead reports dryDaysAtEnd \u2014 how far the dry run had reached at the earlier of the bucket's last day and yesterday \u2014 so a fully dry stretch is not silent. It is only produced for buckets with no drinking day: inside a bucket that does contain one it would measure where the calendar boundary fell rather than anything about the drinking. Drinking on 1 and 28 June would report 2, which is true and useless."),
  bullet("Rest scoring ignores the cross-activity filter \u2014 a dry day means a day with no drinking at all, whatever the record was tagged with. This is why a filtered request issues one extra query."),
  bullet("The SUMMARY view uses the same scoring, so the two screens cannot disagree. Its avgRestDays is likewise nullable and it carries dryDaysAtEnd for a fully dry period."),
);
C(note("Why this replaced the old model: the previous score(D) = diffDays(D, lastDrinkBeforeD) - 1 + (D is rest ? 1 : 0) scored EVERY calendar day, which spread one dry run across many histogram cases and diluted the average \u2014 a Sunday-only week averaged 3.86 instead of 6. It also needed the full drinking history to score any day. The old function survives as computeDailyScoresLegacy, called ONLY by the old mode=trend path; delete both together."));
C(note("Dilution is deliberate, not a defect. Dry through June and July, then drinking every day in August, puts the whole 61-day run on 1 August: August averages 61/31 \u2248 2. The long run earns credit and the 31 drinking days earn a penalty, and the net is the honest figure. A second longest-gap line was considered and rejected for this reason."));
...
C(spacer());
C(bold("Trend API — metric=drinking.trend (v4.6)"));
C(p("Weight-style window: grain (day/week/month) \u00d7 bucket count, anchored at min(end of the selected filter period, today) with the start snapped to a bucket boundary, resolved in the route via trend-window.ts and passed down. computeDrinkingTrend runs its OWN queries rather than taking documents, because it needs the conversion table and the prior-drink lookup anyway."));
C(
  bullet("THREE bounded queries per request \u2014 the conversion table, the one drinking day before the window, and every record in the window \u2014 plus a fourth only when a cross-activity filter is set, since rest scoring must see unfiltered drinking days. None grows with the log."),
  bullet("This replaces three queries PER BUCKET, one of them an unbounded scan of every drinking record ever logged: 360 queries and 120 full scans at 120 buckets. Everything older than the window's prior drinking day cannot affect any score inside it."),
  bullet("Per-bucket fields: label, start, daysInBucket, drinkingDays, totalDrinks, avgDrinksPerDay, drinksBox {min/max/avg}, avgRestDays (nullable), dryDaysAtEnd (nullable), histogram, drinkType, occasions, companions, people, avgStartMins, avgEndMins, avgDurationSeconds."),
  bullet("QUARTILES DROPPED from drinksBox \u2014 the Amt(day) tab draws average, max and min as three lines, and a box per bucket is unreadable at 120 buckets."),
  bullet("The window stops at YESTERDAY: today is partial and would drag every per-day figure down. The fetch is padded by the 6-hour boundary on BOTH sides, the late side fixing an older gap where a post-midnight session on the last day was dropped."),
  bullet("The old mode=trend path and computeDrinkingTrendBucket are untouched and still work; retire them with computeDailyScoresLegacy once the UI no longer calls them."),
);
C(spacer());
C(bold("Legacy Trend Mode API — drinking.summary&mode=trend"));
...
C(
  ...
  bullet("(v4.6) Controls are grain \u00d7 count Segmented pairs, with the option lists and defaults IMPORTED from WeightTrendView rather than copied: 14/30/60/90 \u00b7 12/26/52/104 \u00b7 12/24/60/120, defaults 30/26/24, the count resetting to the grain's default on a grain switch. Month \u00d7 24 is the default view."),
  bullet("The PEOPLE tab keeps its own short 3/6/12 count and its rank-flow chart \u2014 rank flow cannot render at 120 buckets \u2014 and remembers it, so switching to People and back does not destroy a longer window. One shared grain control; only the count list differs.", 1),
  bullet("The Trend toggle is NO LONGER disabled in Period mode, matching Weight and Exercise: the period only anchors the window now.", 1),
  bullet("The resolved-range line formats from the grain the SERVER echoed, never from the control \u2014 the clean fix for the one-frame label-grain mismatch deferred in Exercise.", 1),
  bullet("(v4.6) Chart conversions: Amt(day) box plot \u2192 three CssTrendChart lines (Avg / Max / Min, average in the strong colour, max and min lighter; quartiles dropped by choice). Type / Occasion / Relation stacked bars \u2192 stacked areas, band order fixed across the whole window by total size, largest at the bottom, a zero-total bucket rendering as a gap. Rest \u2192 stacked area of the seven fixed bands as COUNTS, with the rest line on its own right-hand axis in days, solid where a dry run ended and dashed through the dryDaysAtEnd markers across a dry stretch. Session \u2192 the From/To lines and their band fill kept, the arrows and captions dropped, an average Duration line added on its own right-hand axis."),
  bullet("Dash means \"no dry run ended here\", NOT \"different scale\". The two axes are told apart by their labels and the line colour. An earlier plan had the rest line itself dashed to mark the second axis; it was superseded when the dry-stretch bridge needed the dash more.", 1),
);
C(bold("Filters"));
